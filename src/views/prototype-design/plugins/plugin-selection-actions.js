import { deepCopyComponent } from "@/views/prototype-design/utils"
import { EVENT_COMPONENT_DELETE, EVENT_COMPONENT_DUPLICATE } from "@/views/prototype-design/event-enum"
import eventBus from "@/views/prototype-design/utils/eventBus";

export function registerSelectionActions(selection) {
    eventBus.$on(EVENT_COMPONENT_DELETE, () => {
    ...
    eventBus.$on(EVENT_COMPONENT_DUPLICATE, () => {
        if (selection.componentRefs.length > 1) {
            const components = []
            const transformList = []
